...
async def cmd_start(m: types.Message):
    ...
    status = "⭐ Reseller" if is_reseller else "👤 Client"
...
async def specials_menu(call: types.CallbackQuery):
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text="💎 Pack All-In-One", callback_data="buy_special_1"))
    builder.row(types.InlineKeyboardButton(text="🎁 Weekend Pass", callback_data="buy_special_2"))
    builder.row(types.InlineKeyboardButton(text="⬅️ ÎNAPOI", callback_data="home"))
    await call.message.edit_text("🔥 **OFERTE LIMITATE (BEST VALUE):**", reply_markup=builder.as_markup())
...
async def reseller_menu(call: types.CallbackQuery):
    ...
    if is_reseller:
        ...
        text = "💼 **VREI SĂ DEVII RESELLER?**\n\nBeneficii:\n✅ Prețuri cu 40% mai mici\n✅ Suport prioritar\n✅ Acces la aplicații beta\n\n💰 Taxă activare: `100 LEI`"
    
    builder = InlineKeyboardBuilder()
    if not is_reseller:
        builder.row(types.InlineKeyboardButton(text="🚀 Aplică / Activează", url="https://t.me/zenoficiall"))
    builder.row(types.InlineKeyboardButton(text="⬅️ ÎNAPOI", callback_data="home"))
...
async def add_info(call: types.CallbackQuery):
    text = (
        "💳 **METODE DE PLATĂ:**\n\n"
        "📍 **Revolut / PayPal / Crypto**\n\n"
        "1. Trimite suma la @zenoficiall\n"
        "2. Trimite screenshot cu dovada\n"
        "3. Menționează ID-ul tău: `" + str(call.from_user.id) + "`\n\n"
        "⚡ Banii apar în cont imediat după verificare!"
    )
    builder = InlineKeyboardBuilder()
    builder.row(types.InlineKeyboardButton(text="⬅️ Înapoi", callback_data="home"))
    await call.message.edit_text(text, reply_markup=builder.as_markup(), parse_mode="Markdown")
...
async def back_home(call: types.CallbackQuery):
    ...
    builder.row(types.InlineKeyboardButton(text="💳 ADAUGĂ BANI", callback_data="add_info"))
    builder.row(types.InlineKeyboardButton(text="📞 SUPPORT", url="https://t.me/zenoficiall"))
    await call.message.edit_text(f"🏪 **BLESSED PANELS**\n\n🦁 Balanță: `{bal} LEI`\n🏆 Status: `{status}`", reply_markup=builder.as_markup(), parse_mode="Markdown")
...
async def add_money(m: types.Message):
    if m.from_user.id == ADMIN_ID:
        try:
            ...
            cursor.execute("INSERT OR REPLACE INTO users (user_id, balance, is_reseller) VALUES (?, ?, (SELECT is_reseller FROM users WHERE user_id = ?))", (target_id, new_bal, target_id))
            conn.commit()
            await m.answer(f"✅ Adăugat `{suma} LEI` lui `{target_id}`.")
...
async def set_reseller(m: types.Message):
    if m.from_user.id == ADMIN_ID:
        try:
            ...
            cursor.execute("UPDATE users SET is_reseller = 1 WHERE user_id = ?", (target_id,))
            conn.commit()
            await m.answer(f"✅ Utilizatorul `{target_id}` este acum RESELLER!")
