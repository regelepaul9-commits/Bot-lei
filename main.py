...
# Init Tables & Ensure is_admin column exists
...
def main_kb(uid):
    ...
    # Admin check: ori ești Owner, ori ai is_admin = 1 în DB
...
# --- HANDLER SETADMIN (REPARAT) ---
@dp.message(Command("setadmin"))
async def set_admin_handler(m: types.Message):
    if m.from_user.id != OWNER_ID:
        return # Doar owner-ul poate executa asta

    try:
        parts = m.text.split()
        if len(parts) < 2:
            return await m.answer("❌ Folosește: `/setadmin ID_USER`")
        
        target_id = int(parts[1])
        
        # Verificăm dacă user-ul există în DB, dacă nu, îl creăm
        get_u(target_id)
        
        # Update rol admin
        db_query("UPDATE users SET is_admin = 1 WHERE user_id = ?", (target_id,))
        
        await m.answer(f"✅ **SUCCES!**\nUtilizatorul `{target_id}` a fost promovat la rangul de **ADMIN**.\nAcesta poate acum folosi comenzile `/add` și `/addkey`.")
        
        # Trimitem notificare și noului admin
        try:
            await bot.send_message(target_id, "🎊 **FELICITĂRI!**\nAi primit acces de **ADMIN** în Blessed Panels.\nFolosește /start pentru a vedea panoul.")
        except: pass
            
    except ValueError:
        await m.answer("❌ ID-ul trebuie să fie un număr!")
    except Exception as e:
        await m.answer(f"❌ Eroare: {str(e)}")

# --- RESTUL HANDLERELOR (START, SHOP, ADMIN PANEL) ---
...
async def buy_logic(c: types.CallbackQuery):
    # Logica de cumparare ramane neschimbata fata de versiunea anterioara
    pass # (Aici este functia buy_logic pe care am facut-o anterior)
