...
# Prețuri (d1=Drip 1z, d7=Drip 7z, f1=Fluorite 1z, f7=Fluorite 7z)
PRICES = {"d1": 20, "d7": 50, "f1": 25, "f7": 60, "wknd": 15}
...
async def specials_menu(call: types.CallbackQuery):
    ...
    builder.row(types.InlineKeyboardButton(text="⬅️ ÎNAPOI", callback_data="home"))
    await call.message.edit_text("🔥 **OFERTE SPECIALE**\n\n🎫 Weekend Pass: Reducere **5 LEI** la orice cheie!", reply_markup=builder.as_markup())
...
async def add_key_adm(m: types.Message):
    ...
    except: await m.answer("`/addkey tip cheie` (d1, d7, f1, f7)")
...
async def set_reseller(m: types.Message):
    if m.from_user.id == ADMIN_ID:
        try:
            ...
            await m.answer(f"⭐ User {tid} promovat!")
        except: pass
